#ifndef NEO_HOOKEAN_MATERIAL_PARAMETERS_HPP
#define NEO_HOOKEAN_MATERIAL_PARAMETERS_HPP
...
class NeoHookeanMaterialParameters : public MaterialParameters<dim> {
public:
    NeoHookeanMaterialParameters(
        std::function<double(const dealii::Point<dim>&)> shearModulusFun_, 
        std::function<double(const dealii::Point<dim>&)> bulkModulusFun_
        ) : shearModulusFun(shearModulusFun_), bulkModulusFun(bulkModulusFun_), 
            shearModulusLocal(0.0), bulkModulusLocal(0.0)
    { }
    ...
    void computeLocal(const dealii::Point<dim> &point, unsigned int) override {
        shearModulusLocal = shearModulusFun(point);
        bulkModulusLocal = bulkModulusFun(point);
    }
...
private:
    std::function<double(const dealii::Point<dim>&)> shearModulusFun;
    std::function<double(const dealii::Point<dim>&)> bulkModulusFun;
    double shearModulusLocal;
    double bulkModulusLocal;
};
